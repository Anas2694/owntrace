...
- Subscription and exposure views return derived account evidence only. Security-alert and password-reset metadata is never called a verified breach. Verified breach checks are manual opt-in only: OwnTrace sends the signed-in account email to its breach-data service, caches only minimal breach names and safe timestamps for 24 hours, and never sends Gmail data, passwords, or discovered account data.
- The breach-data request uses a fixed server-side endpoint, an encoded account email, a short timeout, no redirects, and safe dependency errors. Source responses, request URLs, and account emails are never logged or returned.
- The free breach-check allowance is process-local for the current single-instance beta: 90 outbound checks per day, 20 per hour, and one per second. Cached results do not use it. Horizontal scaling requires a shared atomic limiter before deployment.
