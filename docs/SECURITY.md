...
- Disconnect first attempts provider revocation, then removes the local connection, sync job, derived Gmail signals, Gmail-derived account evidence, subscription records, actions for deleted accounts, and accounts with no remaining evidence. Network revocation failures retain local data so the user can safely retry.
...
- Subscription detection stores bounded structured facts derived from minimized sender, subject, and date metadata. Amounts require an explicit or unambiguous currency, renewals are labeled estimates, marketing-only signals are excluded, and records never claim active billing. Security-alert and password-reset metadata is never called a verified breach. Verified breach checks are manual opt-in only: OwnTrace tells the user that it sends the signed-in account email to XposedOrNot, caches only minimal breach names and safe timestamps for 24 hours, and never sends Gmail data, passwords, or discovered account data.
